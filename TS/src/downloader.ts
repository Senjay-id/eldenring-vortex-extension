...
import { IPluginRequirement, IGithubDownload, IGitHubAsset } from './types';
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (release.assets.length > 0) {
        const chosenAsset = release.assets.find((asset: IGitHubAsset) => requirement.assetMatcher(asset));
